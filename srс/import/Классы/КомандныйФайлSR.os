...
//
Функция Создать(Знач Путь = "", Знач Расширение = "", Кодировка = Неопределено) Экспорт
...
//
Функция СуффиксПеренаправленияВывода(Знач ИмяФайлаПриемника, 
										Знач УчитыватьStdErr = Истина, 
...
Процедура УстановитьПриложениеPowerShell() Экспорт
	УстановитьПриложение(ПутьДоPowerShell());
КонецПроцедуры

Процедура СоздатьPowerShell(Кодировка = Неопределено) Экспорт
	Если Кодировка = Неопределено Тогда
		Кодировка = КодировкаТекста.UTF8;		
	КонецЕсли;
	Создать("", РасширениеPowerShell(), Кодировка);	
КонецПроцедуры

Функция ПутьДоPowerShell() // BSLLS:UnusedLocalMethod-off
	Возврат "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe";
КонецФункции

Функция РасширениеPowerShell() // BSLLS:UnusedLocalMethod-off
	Возврат "ps1";
КонецФункции
