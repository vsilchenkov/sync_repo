...
Процедура ВЛог(Знач ТекстСообщения, Ошибка = Ложь, ВТелеграм = Ложь, 
	...
	УбратьвИсключенииСтокуКодаИзСтороннихКлассов(ТекстСообщения);
	...
	Если ВТелеграм Тогда
		Если ЗначениеЗаполнено(ТекстСообщенияТелеграм) Тогда
			УбратьвИсключенииСтокуКодаИзСтороннихКлассов(ТекстСообщенияТелеграм);
...
Процедура ДополнитьОсновныеПараметрыВТексте(Текст) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяБазы) И СтрНайти(Текст, ИмяБазы) = 0 Тогда
		Если СтрНайти(Текст, ПараметрNobase) = 0 Тогда
			...
			Если СтрНайти(Текст, Символы.ПС) Тогда
				Разделитель = Символы.ПС;				
			Иначе
				Разделитель = " ";
			КонецЕсли;

			Текст = Текст + Разделитель + """" + ИмяБазы + """";
...
Процедура УбратьвИсключенииСтокуКодаИзСтороннихКлассов(ОписаниеОшибки)
	
	УбратьВИсключенииСтрокуКода(ОписаниеОшибки, "v8runner");
	УбратьВИсключенииСтрокуКода(ОписаниеОшибки, "ГитРепозиторий");
	
КонецПроцедуры

Процедура УбратьВИсключенииСтрокуКода(ОписаниеОшибки, ИмяКласса)
	
	Если СтрНайти(ОписаниеОшибки, ИмяКласса) = 0 Тогда
		Возврат;	
	КонецЕсли;

	СтрокаГотовая = "";
	Для каждого СтрокаРазделенная Из СтрРазделить(ОписаниеОшибки, "/") Цикл
		Если СтрНайти(СтрокаРазделенная, ИмяКласса) > 0 Тогда
			Продолжить;	
		КонецЕсли;	
...
Функция LOGOS_CONFIG(Позиция = 0, 
	...
	СтрокаКоманды = СтрокаКоманды + "logger.oscript.lib.stream=INFO, stream;"
					...
					+ "logger.oscript.lib.gitrunner=INFO, gitrunner;"
					+ "appender.gitrunner=ВыводЛогаВФайл;"
					+ "appender.gitrunner.file=var\log\gitrunner#Позиция.log;";
